...
func TestBuildPerimeter_Greedy(t *testing.T) {
	assert := assert.New(t)
...
func TestPrepare_Greedy(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyImpl([]model.CircuitVertex{
...
func TestUpdate_Greedy(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyImpl([]model.CircuitVertex{
		// Note: the circuit is sorted by Prepare(), so the indices will change as specified below.
		model2d.NewVertex2D(-15, -15), // Index 0 after sorting
...
func TestUpdate_ShouldNotRemoveAttachedInteriorPointFromPerimeterIfNewEdgeIsCloserThanPreviousEdge_Greedy(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyImpl([]model.CircuitVertex{
		model2d.NewVertex2D(0, 0),
