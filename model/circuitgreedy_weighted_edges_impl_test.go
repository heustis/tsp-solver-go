...
func TestBuildPerimeter_Greedy_WeightedEdges(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyWeightedEdgeImpl([]model.CircuitVertex{
	...
	).(*model.CircuitGreedyWeightedEdgeImpl)
...
func TestPrepare_Greedy_WeightedEdges(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyWeightedEdgeImpl([]model.CircuitVertex{
	...
	assert.NotNil(circuit.GetClosestVertices())
	assert.Len(circuit.GetClosestVertices(), 0)
...
func TestUpdate_Greedy_WeightedEdges(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyWeightedEdgeImpl([]model.CircuitVertex{
		// Note: the circuit is sorted by Prepare(), so the indices will change as specified below.
		model2d.NewVertex2D(-15, -15), // Index 0 after sorting
...
func TestUpdate_ShouldNotRemoveAttachedInteriorPointFromPerimeterIfNewEdgeIsCloserThanPreviousEdge_Greedy_WeightedEdges(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyWeightedEdgeImpl([]model.CircuitVertex{
		model2d.NewVertex2D(0, 0),
...
func TestUpdate_Greedy_WeightedEdges_ShouldPanicIfEdgeIsNotInCircuit(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyWeightedEdgeImpl([]model.CircuitVertex{
		// Note: the circuit is sorted by Prepare(), so the indices will change as specified below.
		model2d.NewVertex2D(-15, -15), // Index 0 after sorting
