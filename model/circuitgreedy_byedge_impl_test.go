...
func TestBuildPerimeter_Greedy_ByEdge(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyByEdgeImpl([]model.CircuitVertex{
...
func TestPrepare_Greedy_ByEdge(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyByEdgeImpl([]model.CircuitVertex{
...
func TestUpdate_Greedy_ByEdge(t *testing.T) {
	assert := assert.New(t)
	circuit := model.NewCircuitGreedyByEdgeImpl([]model.CircuitVertex{
		// Note: the circuit is sorted by Prepare(), so the indices will change as specified below.
		model2d.NewVertex2D(-15, -15), // Index 0 after sorting
