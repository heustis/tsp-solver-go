...
func FindShortestPathNPNoChecks(vertices []tspmodel.CircuitVertex) ([]tspmodel.CircuitVertex, float64) {
...
func FindShortestPathNPWithChecks(vertices []tspmodel.CircuitVertex) ([]tspmodel.CircuitVertex, float64) {
	numVertices := len(vertices)
	if numVertices == 0 {
		return nil, 0.0
	}
	...
	_, isGraph := vertices[0].(*tspgraph.GraphVertex)
	...
	for current := treeRoot; treeRoot.shortestPath == nil; {
		...
		if current.depth == numVertices-1 {
			...
			if !isGraph && current.intersectsAnyAncestorsOrIsLongerThanBestLength(vertices, shortestPathLen) {
				...
				current.shortestPath = []tspmodel.CircuitVertex{}
				current = current.parent
			} else {
				current.createChildren(numVertices)
			}
...
type treeNodeTSP struct {
	...
	shortestPath       []tspmodel.CircuitVertex
}
...
func (t *treeNodeTSP) computeLeafPath(vertices []tspmodel.CircuitVertex) {
	// Only allow for leaf nodes to compute the path
	if t.depth != len(vertices)-1 {
		return
	}

	t.shortestPath = make([]tspmodel.CircuitVertex, len(vertices))

	var previousVertex tspmodel.CircuitVertex
	currentNode := t
	pathLength := 0.0
...
func (t *treeNodeTSP) intersectsAnyAncestorsOrIsLongerThanBestLength(vertices []tspmodel.CircuitVertex, bestLength float64) bool {
