...
func FindShortestPathNPHeap(vertices []tspmodel.CircuitVertex) ([]tspmodel.CircuitVertex, float64) {
	// Prepare root of tree
	treeRoot := createHeapNode(0, 1, nil)
	numVertices := len(vertices)

	heap := tspmodel.NewHeap(func(a interface{}) float64 {
	...
	path := make([]tspmodel.CircuitVertex, numVertices)
...
func (t *treeHeapNodeTSP) computePathLen(vertices []tspmodel.CircuitVertex) float64 {
	if t.parent == nil {
		return 0.0
