...
func (v *Vertex3D) DistanceTo(other tspmodel.CircuitVertex) float64 {
	o := other.(*Vertex3D)
...
func (v *Vertex3D) EdgeTo(end tspmodel.CircuitVertex) tspmodel.CircuitEdge {
...
func (v *Vertex3D) Equals(other interface{}) bool {
	if v == other {
		...
		return math.Abs(v.X-otherVertex.X) < tspmodel.Threshold && math.Abs(v.Y-otherVertex.Y) < tspmodel.Threshold && math.Abs(v.Z-otherVertex.Z) < tspmodel.Threshold
	} else {
		return false
	}
}
...
var _ tspmodel.CircuitVertex = (*Vertex3D)(nil)
