...
func TestBuildPerimeter_ShouldProduceSameResultAs2D(t *testing.T) {
	...
	vertices := []tspmodel.CircuitVertex{
		...
		tspmodel3d.NewVertex3D(-15, -15, 0),
		tspmodel3d.NewVertex3D(0, 0, 0),
		tspmodel3d.NewVertex3D(15, -15, 0),
		tspmodel3d.NewVertex3D(3, 0, 0),
		tspmodel3d.NewVertex3D(3, 13, 0),
		tspmodel3d.NewVertex3D(8, 5, 0),
		tspmodel3d.NewVertex3D(9, 6, 0),
		tspmodel3d.NewVertex3D(-7, 6, 0),
	}

	vertices = tspmodel.DeduplicateVerticesNoSorting(vertices)

	circuitEdges, unattachedVertices := tspmodel3d.BuildPerimiter(vertices)
...
func TestBuildPerimeter3D(t *testing.T) {
	...
	vertices := []tspmodel.CircuitVertex{
		// Note: 3d circuits are not sorted.
		tspmodel3d.NewVertex3D(-15, -15, 0),
		tspmodel3d.NewVertex3D(0, 0, 13),
		tspmodel3d.NewVertex3D(15, -15, 5),
		tspmodel3d.NewVertex3D(3, 0, -3),
		tspmodel3d.NewVertex3D(3, 13, -16),
		tspmodel3d.NewVertex3D(8, 5, 4),
		tspmodel3d.NewVertex3D(9, 6, -10),
		tspmodel3d.NewVertex3D(-7, 6, -8),
	}
