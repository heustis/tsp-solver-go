...
func (e *Edge3D) DistanceIncrease(vertex tspmodel.CircuitVertex) float64 {
...
func (e *Edge3D) GetStart() tspmodel.CircuitVertex {
	return e.Start
}

// GetEnd returns the end vertex of the edge
func (e *Edge3D) GetEnd() tspmodel.CircuitVertex {
	return e.End
}
...
func (e *Edge3D) Intersects(other tspmodel.CircuitEdge) bool {
	...
	if math.Abs(denominator) < tspmodel.Threshold {
		...
		return math.Abs(startToStartDenominator) < tspmodel.Threshold && (tspmodel.IsBetween(e.Start.X, otherEdge3D.Start.X, otherEdge3D.End.X) ||
			tspmodel.IsBetween(e.End.X, otherEdge3D.Start.X, otherEdge3D.End.X) ||
			tspmodel.IsBetween(otherEdge3D.Start.X, e.Start.X, e.End.X) ||
			tspmodel.IsBetween(otherEdge3D.End.X, e.Start.X, e.End.X))
	}
	...
	if percentE < -tspmodel.Threshold || percentE > 1.0+tspmodel.Threshold {
		return false
	}
	...
	// If the closest point is not within the the start and end points, then the line segments do not intersect, even if the infinite lines do.
	if percentOther < -tspmodel.Threshold || percentOther > 1.0+tspmodel.Threshold {
		return false
	}
...
func (e *Edge3D) Merge(other tspmodel.CircuitEdge) tspmodel.CircuitEdge {
...
func (e *Edge3D) Split(vertex tspmodel.CircuitVertex) (tspmodel.CircuitEdge, tspmodel.CircuitEdge) {
...
var _ tspmodel.CircuitEdge = (*Edge3D)(nil)
