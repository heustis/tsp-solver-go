...
func BuildPerimiter(verticesArg []tspmodel.CircuitVertex) ([]tspmodel.CircuitEdge, map[tspmodel.CircuitVertex]bool) {
	numVertices := len(verticesArg)
	midpoint := NewVertex3D(0, 0, 0)
	unattachedVertices := make(map[tspmodel.CircuitVertex]bool)
	distanceToMidpoint := make(map[tspmodel.CircuitVertex]float64)
	exteriorClosestEdges := make(map[tspmodel.CircuitVertex]*tspmodel.DistanceToEdge)
	circuitEdges := make([]tspmodel.CircuitEdge, 0, numVertices)
	...
	farthestFromMid := tspmodel.FindFarthestPoint(midpoint, verticesArg)
	...
	farthestFromFarthest := tspmodel.FindFarthestPoint(farthestFromMid, verticesArg)
	...
	for vertex := range unattachedVertices {
		...
		exteriorClosestEdges[vertex] = &tspmodel.DistanceToEdge{
			Edge:     e3d,
	...
	for len(exteriorClosestEdges) > 0 {
		...
		circuitEdges, _ = tspmodel.SplitEdge(circuitEdges, farthestFromClosestEdge.Edge, farthestFromClosestEdge.Vertex)
		...
		for v := range unattachedVertices {
			...
			if closest, wasExterior := exteriorClosestEdges[v]; wasExterior {
				...
				v3d := v.(*Vertex3D)
				newClosest := &tspmodel.DistanceToEdge{
					Vertex:   v3d,
...
func isInterior(v tspmodel.CircuitVertex, closestEdge tspmodel.CircuitEdge, midpoint *Vertex3D, distanceToMidpoint float64) bool {
...
var _ tspmodel.PerimeterBuilder = BuildPerimiter
