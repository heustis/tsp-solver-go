...
func BuildPerimiter(verticesArg []tspmodel.CircuitVertex) (circuitEdges []tspmodel.CircuitEdge, unattachedVertices map[tspmodel.CircuitVertex]bool) {
	...
	farthestFromMid := tspmodel.FindFarthestPoint(midpoint, verticesArg).(*Vertex2D)
	...
	farthestFromFarthest := tspmodel.FindFarthestPoint(farthestFromMid, verticesArg).(*Vertex2D)
	...
	for len(exteriorClosestEdges) > 0 {
		...
		var edgeIndex int
		circuitEdges, edgeIndex = tspmodel.SplitEdge(circuitEdges, farthestFromClosestEdge.Edge, farthestFromClosestEdge.Vertex)
		...
		for v := range unattachedVertices {
			...
			if closest, wasExterior := exteriorClosestEdges[v]; wasExterior && closest.Edge == farthestFromClosestEdge.Edge {
				var newClosest *tspmodel.DistanceToEdge
