...
func (h *Heap) TrimN(numberToRetain int) {
	if numberToRetain >= h.Len() || numberToRetain < 0 {
		return
	}
...
// entryString converts an entry in the heap to a string.
func entryString(value interface{}) string {
	if p, okay := value.(fmt.Stringer); okay {
		return p.String()
