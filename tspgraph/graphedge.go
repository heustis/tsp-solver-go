...
// checking each vertex in the tspgraph, which are sorted by distance from the start vertex, for early escape.
// If a path cannot be created from the start vertex to the end vertex nil will be returned (the tspgraph is asymmetric, so it is possible to connect only one way).
func NewGraphEdge(start *GraphVertex, end *GraphVertex) *GraphEdge {
	visited := make(map[*GraphVertex]bool)

	toVisit := tspmodel.NewHeap(func(a interface{}) float64 {
...
func (e *GraphEdge) Delete() {
	// Note: Deleting a tspgraph edge should not delete the tspgraph itself, use Graph.Delete() to delete all GraphVertexes.
	e.path = nil
}

func (e *GraphEdge) DistanceIncrease(vertex tspmodel.CircuitVertex) float64 {
	a, b := e.Split(vertex)
...
func (e *GraphEdge) GetEnd() tspmodel.CircuitVertex {
...
func (e *GraphEdge) GetStart() tspmodel.CircuitVertex {
	return e.path[0]
}

func (e *GraphEdge) Intersects(other tspmodel.CircuitEdge) bool {
...
func (e *GraphEdge) Merge(other tspmodel.CircuitEdge) tspmodel.CircuitEdge {
	return NewGraphEdge(e.GetStart().(*GraphVertex), other.GetEnd().(*GraphVertex))
}

func (e *GraphEdge) Split(vertex tspmodel.CircuitVertex) (tspmodel.CircuitEdge, tspmodel.CircuitEdge) {
...
var _ tspmodel.CircuitEdge = (*GraphEdge)(nil)
