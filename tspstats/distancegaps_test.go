...
func TestCloneDistanceGaps(t *testing.T) {
	...
	statsA := &tspstats.DistanceGaps{
		ClosestEdges: []*tspmodel.DistanceToEdge{
			{
				Vertex:   tspmodel2d.NewVertex2D(2, 3),
				Edge:     tspmodel2d.NewEdge2D(tspmodel2d.NewVertex2D(1, 1), tspmodel2d.NewVertex2D(3, 4)),
				Distance: 3.45,
			},
			{
				Vertex:   tspmodel2d.NewVertex2D(-6, 7),
				Edge:     tspmodel2d.NewEdge2D(tspmodel2d.NewVertex2D(1, -7), tspmodel2d.NewVertex2D(-8, 9)),
				Distance: 4.56,
			},
			{
				Vertex:   tspmodel2d.NewVertex2D(0, 0),
				Edge:     tspmodel2d.NewEdge2D(tspmodel2d.NewVertex2D(1, -7), tspmodel2d.NewVertex2D(-8, 9)),
				Distance: 7.89,
			},
		},
	...
	statsA.ClosestEdges = append(statsA.ClosestEdges, &tspmodel.DistanceToEdge{
		Vertex:   tspmodel2d.NewVertex2D(10, 10),
		Edge:     tspmodel2d.NewEdge2D(tspmodel2d.NewVertex2D(1, -7), tspmodel2d.NewVertex2D(-8, 9)),
		Distance: 17.89,
	})
	...
	statsB.ClosestEdges = append(statsB.ClosestEdges, &tspmodel.DistanceToEdge{
		Vertex:   tspmodel2d.NewVertex2D(15, 15),
		Edge:     tspmodel2d.NewEdge2D(tspmodel2d.NewVertex2D(3, 3), tspmodel2d.NewVertex2D(40, 40)),
		Distance: 0.0,
	})
...
func TestNewDistanceGaps(t *testing.T) {
	...
	stats1 := tspstats.NewDistanceGaps(vertices[1], edges)
	assert.NotNil(stats1)
	assert.Len(stats1.ClosestEdges, 5)
	assert.Len(stats1.Gaps, 4)

	assert.Equal(stats1.ClosestEdges[0], &tspmodel.DistanceToEdge{
		Vertex:   vertices[1],
		Edge:     edges[4],
		Distance: 7.9605428386450825,
	})
	assert.Equal(stats1.ClosestEdges[1], &tspmodel.DistanceToEdge{
		Vertex:   vertices[1],
		Edge:     edges[1],
		Distance: 10.189527594146842,
	})
	assert.Equal(stats1.ClosestEdges[2], &tspmodel.DistanceToEdge{
		Vertex:   vertices[1],
		Edge:     edges[3],
		Distance: 10.35465290568552,
	})
	assert.Equal(stats1.ClosestEdges[3], &tspmodel.DistanceToEdge{
		Vertex:   vertices[1],
		Edge:     edges[0],
		Distance: 12.426406871192853,
	})
	assert.Equal(stats1.ClosestEdges[4], &tspmodel.DistanceToEdge{
		Vertex:   vertices[1],
		Edge:     edges[2],
	...
	stats3 := tspstats.NewDistanceGaps(vertices[3], edges)
	assert.NotNil(stats3)
	assert.Len(stats3.ClosestEdges, 5)
	assert.Len(stats3.Gaps, 4)

	assert.Equal(stats3.ClosestEdges[0], &tspmodel.DistanceToEdge{
		Vertex:   vertices[3],
		Edge:     edges[1],
		Distance: 5.854324418695558,
	})
	assert.Equal(stats3.ClosestEdges[1], &tspmodel.DistanceToEdge{
		Vertex:   vertices[3],
		Edge:     edges[2],
		Distance: 12.26573691694568,
	})
	assert.Equal(stats3.ClosestEdges[2], &tspmodel.DistanceToEdge{
		Vertex:   vertices[3],
		Edge:     edges[3],
		Distance: 12.4553481739569,
	})
	assert.Equal(stats3.ClosestEdges[3], &tspmodel.DistanceToEdge{
		Vertex:   vertices[3],
		Edge:     edges[4],
		Distance: 12.620447763166336,
	})
	assert.Equal(stats3.ClosestEdges[4], &tspmodel.DistanceToEdge{
		Vertex:   vertices[3],
		Edge:     edges[0],
...
func TestStringDistanceGaps(t *testing.T) {
	...
	vertexLookup := make(map[tspmodel.CircuitVertex]int)
	...
	edgeLookup := make(map[tspmodel.CircuitEdge]int)
	for i, e := range edges {
		edgeLookup[e] = i
	}

	assert.Equal("{}", (&tspstats.DistanceGaps{}).String(vertexLookup, edgeLookup))
...
func TestUpdateStatsShouldUpdateStartOfList(t *testing.T) {
	...
	assert.Equal(stats1.ClosestEdges[5], &tspmodel.DistanceToEdge{
		Vertex:   vertices[1],
		Edge:     edges[2],
		Distance: 14.938773433225418,
	})
