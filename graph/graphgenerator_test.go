	...
	for i := 0; i < 10; i++ {
		for j := 0; j < 10; j++ {
			if j != i {
				...
				assert.NotNil(e, fmt.Sprintf(`cannot create path fromIndex=%d toIndex=%d graph=%s`, i, j, g.ToString()))
			}
		}
	}
}
...
func TestCreate_ShouldProduceVerticesWithoutDuplicateNames(t *testing.T) {
	...
	for _, v := range g.Vertices {
		assert.True(expectedNames[v.GetId()], fmt.Sprintf(`unexpected or duplicate name for vertex, name=%s graph=%s`, v.GetId(), g.ToString()))
...
func TestCreate_ShouldProduceSameGraphForSameSeed(t *testing.T) {
	...
	for seed = -5; seed < 5; seed++ {
		...
		assert.Equal(g.ToString(), g2.ToString(), seed)
	...
	for min := 2; min < 10; min++ {
		for max := 0; max < 10; max++ {
			...
			for _, v := range g.Vertices {
				...
				assert.GreaterOrEqual(len(adj), min, fmt.Sprintf(`vertex has too few adjacent vertices, name=%s graph=%s`, v.GetId(), g.ToString()))
				for other := range adj {
					symmetricDist, symmetricEdgeExists := other.GetAdjacentVertices()[v]
					assert.True(symmetricEdgeExists, fmt.Sprintf(`vertex has path without symmetric return path, sourceVertex=%s destinationVertex=%s graph=%s`, v.GetId(), other.GetId(), g.ToString()))
...
func TestCreate_ShouldProduceAsymmetricEdgesIfEnabled(t *testing.T) {
	...
	for _, v := range g.Vertices {
		...
		for other := range adj {
			...
			assert.True(returnEdgeExists, fmt.Sprintf(`vertex has path without return path, sourceVertex=%s destinationVertex=%s graph=%s`, v.GetId(), other.GetId(), g.ToString()))
			if !hasAsymmetricEdge {
				hasAsymmetricEdge = returnDist != adj[other]
			}
		}
	}
	assert.True(hasAsymmetricEdge, fmt.Sprintf(`graph did not generate asymetric edges though it was configured to %s`, g.ToString()))
...
func TestCreate_ShouldProduceUnidirectionalEdgesIfEnabled(t *testing.T) {
	...
	assert.True(hasUnidirectionalEdge, fmt.Sprintf(`graph did not generate unidirectional edges though it was configured to %s`, g.ToString()))
